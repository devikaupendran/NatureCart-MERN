    ...
    return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            <div className="hidden sm:flex items-center gap-8">
                ...
                {
                    !user ? (
                        <button className="cursor-pointer px-8 py-2 bg-green-700 hover:bg-green-600 transition text-white rounded-full" onClick={() => setShowUserLogin(true)}>
                            Login
                        </button>
                    ) : (
            ...
            {/* ------------------- Mobile Menu ------------------- */}
            {
                open && (
                    <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] z-2 left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
                        ...
                        {/* if there is user then only show my orders option in menubar */}
